...
CREATE INDEX message_idx_is_time_sensitive ON message (is_time_sensitive)
WHERE
  is_time_sensitive = 1;
...
CREATE INDEX message_idx_fallback_hash ON message (fallback_hash)
WHERE
  fallback_hash IS NOT NULL;
...
CREATE INDEX message_idx_is_pending_satellite_message ON message (is_pending_satellite_send)
WHERE
  is_pending_satellite_send = 1;
...
CREATE INDEX attachment_idx_is_sticker_pgen_state ON attachment (is_sticker, filename, preview_generation_state)
WHERE
  is_sticker = 1;

CREATE INDEX message_idx_pending_indexing_messages ON message (rowid)
WHERE
  index_state = 1
  or index_state = 3;

CREATE INDEX persistent_tasks_report ON persistent_tasks (flag, flag_group, lane, reason, retry_count)
WHERE
  retry_count < 5;

CREATE INDEX chat_message_join_idx_metrics_rebuild ON chat_message_join (index_state)
WHERE
  index_state = 0
  OR index_state = 1
  OR index_state = 3;

CREATE INDEX message_idx_invalid_index_state ON message (rowid)
WHERE
  index_state != 2
  AND (
    (
      (associated_message_type not between 2000 and 2007)
...
CREATE INDEX message_idx_indexed_messages_guid ON message (guid)
WHERE
  index_state = 1;
...
WHERE
  person_centric_id IS NOT NULL;

CREATE TRIGGER update_message_date_after_update_on_message AFTER
UPDATE OF date ON message BEGIN
...
CREATE TRIGGER before_deleting_chat_delete_chat_background_trigger BEFORE DELETE ON chat BEGIN
SELECT
  delete_chat_background_before_deleting_chat (OLD.guid, OLD.properties);
...
CREATE TRIGGER verify_chat_update BEFORE
UPDATE OF guid ON chat BEGIN
SELECT
  verify_chat (NEW.guid);
...
CREATE TRIGGER update_last_failed_message_date AFTER
UPDATE OF error ON message WHEN NEW.error != 0
...
CREATE TRIGGER verify_chat_insert BEFORE INSERT ON chat BEGIN
SELECT
  verify_chat (NEW.guid);

END;

CREATE TRIGGER chat_service_on_insert_chat_message_join AFTER INSERT ON chat_message_join BEGIN
INSERT INTO
...
FROM
  message m
WHERE
  m.rowid = NEW.message_id
ON CONFLICT (service, chat) DO NOTHING;

END;

CREATE TRIGGER before_delete_chat_update_sync_chat_deletes BEFORE DELETE ON chat WHEN is_mic_enabled () = 1 BEGIN
INSERT INTO
  sync_deleted_chats (guid, recordID, timestamp)
SELECT
  guid_for_chat (OLD.chat_identifier, scl.service_name, OLD.style),
  scl.ck_record_id,
  m.date
FROM
  sync_chat_slice scl
...
CREATE TRIGGER message_index_state_propagation AFTER
UPDATE OF index_state ON message BEGIN
UPDATE chat_message_join
SET
  index_state = NEW.index_state
WHERE
  message_id = NEW.rowid;

END;

CREATE TRIGGER message_index_state_clear AFTER DELETE ON chat_message_join WHEN OLD.index_state = 1
OR OLD.index_state = 3 BEGIN
UPDATE message
SET
  index_state = 0
WHERE
...
CREATE TRIGGER index_metrics_delete_propagation AFTER DELETE ON chat_message_join WHEN OLD.index_state != 2 BEGIN
UPDATE index_state_metrics
SET
  pending_count = pending_count - CASE
    WHEN OLD.index_state = 0 THEN 1
    ELSE 0
  END,
  donated_count = donated_count - CASE
    WHEN OLD.index_state = 1 THEN 1
    ELSE 0
  END,
  redonation_count = redonation_count - CASE
    WHEN OLD.index_state = 3 THEN 1
    ELSE 0
  END
WHERE
  id = 1;
...
CREATE TRIGGER index_metrics_update_propagation AFTER
UPDATE OF index_state ON chat_message_join BEGIN
UPDATE index_state_metrics
SET
  pending_count = pending_count + COALESCE(
    CASE
      WHEN NEW.index_state = 0 THEN 1
      ELSE 0
    END,
    0
  ) - COALESCE(
  ...
  donated_count = donated_count + COALESCE(
    CASE
      WHEN NEW.index_state = 1 THEN 1
      ELSE 0
    END,
    0
  ) - COALESCE(
  ...
  redonation_count = redonation_count + COALESCE(
    CASE
      WHEN NEW.index_state = 3 THEN 1
      ELSE 0
    END,
    0
  ) - COALESCE(
...
CREATE TRIGGER index_metrics_insert_propagation AFTER INSERT ON chat_message_join WHEN NEW.index_state != 2 BEGIN
UPDATE index_state_metrics
SET
  pending_count = pending_count + CASE
    WHEN NEW.index_state = 0 THEN 1
