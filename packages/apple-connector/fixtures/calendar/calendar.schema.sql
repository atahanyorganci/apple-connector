...
CREATE TABLE Store (
  ...
  last_sync_end REAL,
  delegated_account_owner_store_id TEXT,
  delegated_account_default_calendar_for_new_events_id INTEGER,
  shows_notifications INTEGER,
  flags2 INTEGER,
  display_order INTEGER,
  owner_name TEXT,
  default_all_day_alarm_offset INTEGER,
  error_id INTEGER,
  cached_external_info BLOB,
  app_group_id TEXT UNIQUE
);
...
CREATE TRIGGER delete_store_members AFTER DELETE ON Store BEGIN
...
CREATE TRIGGER delete_store_changes AFTER DELETE ON Store BEGIN
DELETE FROM CalendarChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM CalendarItemChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM AlarmChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM RecurrenceChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM ParticipantChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM AttachmentChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM ContactChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM EventActionChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM ExceptionDateChanges
WHERE
  store_id = OLD.ROWID;
...
DELETE FROM ShareeChanges
WHERE
  store_id = OLD.ROWID;

DELETE FROM ClientCursor
WHERE
  store_id = OLD.ROWID;
...
CREATE INDEX CalendarImageId on Calendar (image_id)
WHERE
  image_id > 0;

CREATE INDEX CalendarChangesSequenceNumber on CalendarChanges (sequence_number);

CREATE TRIGGER delete_calendar_members AFTER DELETE ON Calendar BEGIN
DELETE FROM OccurrenceCacheDays
where
  calendar_id = OLD.ROWID;

DELETE FROM OccurrenceCache
where
  calendar_id = OLD.ROWID;

DELETE FROM CalendarItem
WHERE
  calendar_id = OLD.ROWID;
  ...
  AND (
    NOT EXISTS (
      ...
      FROM
        CalendarItem
      where
        CalendarItem.image_id > 0
        AND CalendarItem.image_id = OLD.image_id
    )
  )
  AND (
    NOT EXISTS (
      ...
      where
        Calendar.image_id > 0
        AND Calendar.image_id = OLD.image_id
    )
  );
...
CREATE TABLE RecurrenceChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  external_id TEXT,
...
CREATE INDEX AlarmsWithLargeOffsets on Alarm (trigger_interval)
WHERE
  has_long_interval = 1;

CREATE INDEX AlarmChangesSequenceNumber on AlarmChanges (sequence_number);

CREATE TRIGGER cascade_alarm_delete AFTER DELETE ON Alarm BEGIN
DELETE FROM Location
WHERE
  alarm_owner_id = OLD.ROWID
  AND alarm_owner_id > 0;
...
CREATE TABLE ParticipantChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  entity_type INTEGER,
...
CREATE TABLE EventActionChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  event_id INTEGER,
...
CREATE TABLE CalendarItemChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  summary INTEGER,
  ...
  travel_time INTEGER,
  start_location_id INTEGER,
  proposed_start_date INTEGER,
  recurrence_set TEXT,
  store_id INTEGER,
  has_dirty_instance_attributes INTEGER,
  old_calendar_id INTEGER,
  old_external_id TEXT,
  has_recurrence_set_changed INTEGER,
  suppress_notification_for_changes INTEGER
);
...
CREATE INDEX CalendarItemImageId on CalendarItem (image_id)
WHERE
  image_id > 0;

CREATE INDEX CalendarItemCreatedById on CalendarItem (created_by_id)
WHERE
  created_by_id > 0;

CREATE INDEX CalendarItemModifiedById on CalendarItem (modified_by_id)
WHERE
  modified_by_id > 0;

CREATE INDEX CalendarItemColorId on CalendarItem (color_id)
WHERE
  color_id > 0;

CREATE INDEX CalendarItemChangesSequenceNumber on CalendarItemChanges (sequence_number);

CREATE TRIGGER delete_event_alarms_recurs AFTER DELETE ON CalendarItem BEGIN
DELETE FROM Location
WHERE
  item_owner_id = OLD.ROWID;

DELETE FROM Location
WHERE
  start_loc_owner_id > 0
  AND start_loc_owner_id = OLD.ROWID;

DELETE FROM Location
WHERE
  end_loc_owner_id > 0
  AND end_loc_owner_id = OLD.ROWID;

DELETE FROM Location
WHERE
  client_loc_owner_id > 0
  AND client_loc_owner_id = OLD.ROWID;

DELETE FROM Alarm
WHERE
  calendaritem_owner_id = OLD.ROWID;
...
DELETE FROM SuggestedEventInfo
WHERE
  ROWID = OLD.suggested_event_info_id;
  ...
  AND (
    NOT EXISTS (
      ...
      FROM
        CalendarItem
      where
        CalendarItem.color_id > 0
        AND CalendarItem.color_id = OLD.color_id
    )
  );
...
CREATE TABLE ExceptionDateChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  owner_id INTEGER,
...
CREATE TABLE AttachmentChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  owner_id INTEGER,
...
CREATE TRIGGER delete_attachment_file AFTER DELETE ON Attachment BEGIN
DELETE FROM AttachmentFile
WHERE
  AttachmentFile.ROWID = OLD.file_id
  AND (
    SELECT
      file_id
    FROM
...
CREATE TRIGGER delete_category AFTER DELETE ON CategoryLink BEGIN
DELETE FROM Category
WHERE
  Category.ROWID = OLD.category_id
  AND NOT EXISTS (
    SELECT
      category_id
    FROM
...
CREATE TABLE Location (
  ...
  start_loc_owner_id INTEGER,
  end_loc_owner_id INTEGER,
  client_loc_owner_id INTEGER
);

CREATE INDEX LocationOwnerItemId on Location (item_owner_id);

CREATE INDEX LocationStartLocOwnerItemId on Location (start_loc_owner_id)
WHERE
  start_loc_owner_id > 0;

CREATE INDEX LocationEndLocOwnerItemId on Location (end_loc_owner_id)
WHERE
  end_loc_owner_id > 0;

CREATE INDEX LocationClientLocOwnerItemId on Location (client_loc_owner_id)
WHERE
  client_loc_owner_id > 0;

CREATE INDEX LocationAlarmOwnerID on Location (alarm_owner_id)
WHERE
  alarm_owner_id > 0;
...
CREATE TRIGGER delete_resource_changes_for_notification AFTER DELETE ON Notification BEGIN
DELETE FROM ResourceChange
WHERE
...
CREATE TRIGGER delete_notification_calendar AFTER DELETE ON ResourceChange BEGIN
DELETE FROM ResourceChange
WHERE
  calendar_id NOT IN (
    SELECT
...
CREATE TABLE SuggestedEventInfo (
  ROWID INTEGER PRIMARY KEY AUTOINCREMENT,
  owner_id INTEGER,
  opaque_key TEXT,
  unique_key TEXT,
  ...
  timestamp REAL,
  extraction_group_identifier TEXT
);
...
CREATE TABLE AuxDatabase (
  ROWID INTEGER PRIMARY KEY AUTOINCREMENT,
  path TEXT,
  persona_id TEXT UNIQUE
);

CREATE TABLE AuxDatabaseChanges (
  record INTEGER,
  type INTEGER,
  sequence_number INTEGER,
  persona_id TEXT
...
CREATE TRIGGER delete_aux_database_accounts AFTER DELETE ON AuxDatabase BEGIN
DELETE FROM AuxDatabaseAccount
WHERE
  database_id = OLD.ROWID;

END;

CREATE TABLE AuxDatabaseAccount (
  ROWID INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE OccurrenceCacheDays (
  calendar_id INTEGER,
  store_id INTEGER,
  day REAL,
  count INTEGER,
...
CREATE TRIGGER update_cache_days_after_delete AFTER DELETE ON OccurrenceCache BEGIN
UPDATE OccurrenceCacheDays
SET
  count = count - 1
WHERE
  day = OLD.day
  AND calendar_id = OLD.calendar_id
  AND OLD.next_reminder_date IS NULL;
...
CREATE TRIGGER update_cache_days_after_insert AFTER INSERT ON OccurrenceCache BEGIN
REPLACE INTO
  OccurrenceCacheDays
VALUES
  (
    ...
    NEW.store_id,
    NEW.day,
    IIF(NEW.next_reminder_date IS NULL, 1, 0) + IFNULL(
      (
        SELECT
          count
        FROM
...
CREATE TRIGGER update_cache_days_after_update AFTER
UPDATE OF day ON OccurrenceCache BEGIN
REPLACE INTO
  OccurrenceCacheDays
VALUES
  (
...
CREATE TABLE AlarmCache (
  ...
  travel_time REAL,
  is_default INTEGER,
  PRIMARY KEY (event_id, alarm_id, occurrence_date)
);

CREATE TRIGGER delete_from_alarm_cache_after_delete_alarm AFTER DELETE ON Alarm BEGIN
DELETE From AlarmCache
WHERE
  alarm_id = OLD.ROWID;
...
CREATE INDEX OccurrenceCacheCalendarIDDayNextReminderDate on OccurrenceCache (calendar_id, day, next_reminder_date)
WHERE
  next_reminder_date IS NOT NULL;

CREATE TABLE ScheduledTaskCache (
  day REAL,
...
CREATE TRIGGER update_task_cache_count_after_insert AFTER INSERT ON ScheduledTaskCache BEGIN
REPLACE INTO
  ScheduledTaskCache
VALUES
  (
...
CREATE TRIGGER update_task_cache_count_after_update AFTER
UPDATE OF day ON ScheduledTaskCache BEGIN
UPDATE ScheduledTaskCache
SET
  count = count - 1
WHERE
  day = OLD.day
...
CREATE TRIGGER update_task_cache_days_after_delete AFTER DELETE ON ScheduledTaskCache BEGIN
UPDATE ScheduledTaskCache
SET
  count = count - 1
WHERE
...
CREATE TABLE ClientCursorConsumed (
  client_identifier TEXT,
...
CREATE TRIGGER delete_clientcursor_consumed AFTER DELETE ON ClientCursor BEGIN
DELETE FROM ClientCursorConsumed
WHERE
  client_identifier = OLD.client_identifier;

DELETE FROM ClientSequence
WHERE
  client_identifier = OLD.client_identifier;
