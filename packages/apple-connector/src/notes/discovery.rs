...
mod tests {
    ...
    fn default_database_path_is_under_group_containers() {
        let path = default_notes_database_path().expect("database path");
        assert!(path.to_string_lossy().contains("group.com.apple.notes"));
        assert!(path.to_string_lossy().ends_with("NoteStore.sqlite"));
    }

    #[test]
    fn default_attachment_root_points_at_accounts() {
        let path = default_notes_attachment_root().expect("attachment root");
        assert!(path.to_string_lossy().contains("group.com.apple.notes"));
