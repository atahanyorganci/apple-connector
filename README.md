...
read-only HTTP API over Messages.app data backed by a live SQLite connection to
...
- Full Disk Access for the terminal that runs the server
...
| `--database <PATH>` | `$HOME/Library/Messages/chat.db` | Read-only path to Messages `chat.db`. The server never creates or mutates this file. |
| `--attachment-root <PATH>` | `<database-dir>/Attachments` | Messages attachment directory (canonicalized and confined at runtime). |
...
cargo run -p apple-connector -- --database /path/to/chat.db
...
- **Full Disk Access** is required to open `chat.db` and attachment files under
  `~/Library/Messages/Attachments/`.
- If the database cannot be opened, `/healthz` returns `503` with
  `{"status":"unavailable"}` and data endpoints return structured
  `service_unavailable` errors. Startup logs do **not** include filesystem
  paths.
...
| --- | --- | --- |
| `GET` | `/healthz` | Database pool health (`ok` / `unavailable`). |
